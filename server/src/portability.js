...
export function assessmentExport(id) {
 const a=db.prepare('SELECT * FROM assessments WHERE id=? AND deleted_at IS NULL').get(id); if(!a)return null;
 const client=db.prepare('SELECT * FROM clients WHERE id=? AND deleted_at IS NULL').get(a.client_id);
 const behaviors=db.prepare('SELECT * FROM target_behaviors WHERE assessment_id=? ORDER BY id').all(id);
 const points=db.prepare('SELECT * FROM data_points WHERE assessment_id=? ORDER BY recorded_at,id').all(id);
 const hypotheses=db.prepare('SELECT * FROM function_hypotheses WHERE assessment_id=? ORDER BY target_behavior_id').all(id).map(h=>{let evidence={};try{evidence=JSON.parse(h.evidence||'{}')}catch{}return {...h,evidence};});
 const sign_offs=db.prepare('SELECT * FROM sign_offs WHERE assessment_id=? ORDER BY id').all(id);
...
export function csvExport(id) {
 const p=assessmentExport(id); if(!p)return null;
...
export function importCsv(id,text) {
 const a=db.prepare('SELECT id FROM assessments WHERE id=? AND deleted_at IS NULL').get(id);if(!a)return {notFound:true};
 ...
 const idx=Object.fromEntries(headers.map((h,i)=>[h,i]));const behaviors=db.prepare('SELECT id,name FROM target_behaviors WHERE assessment_id=?').all(id);const byName=new Map(behaviors.map(b=>[normalize(b.name),b]));
