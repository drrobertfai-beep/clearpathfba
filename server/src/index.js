...
if(db.prepare('SELECT COUNT(*) AS n FROM users').get().n===0){db.prepare('INSERT INTO users(username,password_hash,role,display_name,must_change_password) VALUES (?,?,?,?,1)').run('admin',hashPassword('admin123'),'admin','Administrator');db.prepare('INSERT INTO users(username,password_hash,role,display_name,must_change_password) VALUES (?,?,?,?,1)').run('bcba',hashPassword('admin123'),'bcba','Sam Rivera, BCBA');console.log('Seeded default users: admin / admin123, bcba / admin123 — CHANGE BEFORE PRODUCTION.');}
for(const name of ['admin','bcba']){const seed=db.prepare('SELECT * FROM users WHERE username=?').get(name);if(seed&&!seed.must_change_password&&verifyPassword('admin123',seed.password_hash))db.prepare('UPDATE users SET must_change_password=1 WHERE id=?').run(seed.id);}
...
function validateDataPoint(body, assessmentId) {
 const target = Number(body.target_behavior_id);
 if (!Number.isInteger(target) || target < 1) return 'target_behavior_id is required.';
 const tb = db.prepare('SELECT id FROM target_behaviors WHERE id=? AND assessment_id=?').get(target, assessmentId);
...
function dataPointRow(id) { return db.prepare(`SELECT dp.*, tb.name AS target_behavior_name, tb.is_safety_concern AS target_behavior_safety, tb.safety_classification AS target_behavior_safety_classification FROM data_points dp LEFT JOIN target_behaviors tb ON tb.id=dp.target_behavior_id WHERE dp.id=?`).get(id); }
...
app.use((req,res,next)=>req.path==='/api/health'||req.path==='/api/auth/login'||req.path==='/api/auth/mfa/verify'||req.path.startsWith('/api/verify/sign-off/')?next():requireAuth(req,res,next));
...
app.post('/api/auth/mfa/setup',(req,res)=>{const u=db.prepare('SELECT * FROM users WHERE id=?').get(req.user.id);const secret=generateSecret();db.prepare('UPDATE users SET mfa_secret=? WHERE id=?').run(secret,u.id);res.json({otpauth_uri:otpauthUri(secret,u.username),secret});});
app.post('/api/auth/mfa/enable',(req,res)=>{const u=db.prepare('SELECT * FROM users WHERE id=?').get(req.user.id);if(!u?.mfa_secret)return res.status(400).json({error:'Run MFA setup first.'});if(!verifyTOTP(u.mfa_secret,req.body?.code))return res.status(400).json({error:'Invalid authenticator code.'});const codes=generateBackupCodes();const tx=db.transaction(()=>{db.prepare('DELETE FROM mfa_backup_codes WHERE user_id=?').run(u.id);const ins=db.prepare('INSERT INTO mfa_backup_codes(user_id,code_hash) VALUES(?,?)');codes.forEach(c=>ins.run(u.id,hashBackupCode(c)));db.prepare('UPDATE users SET mfa_enabled=1 WHERE id=?').run(u.id);});tx();logAudit(db,{assessment_id:null,actor:u.username,action:'mfa_enabled',details:{label:`MFA enabled: ${u.username}`,username:u.username}});res.json({mfa_enabled:true,backup_codes:codes});});
app.post('/api/auth/mfa/disable',(req,res)=>{const u=db.prepare('SELECT * FROM users WHERE id=?').get(req.user.id);if(u.role!=='admin'&&!verifyTOTP(u.mfa_secret,req.body?.code))return res.status(400).json({error:'A valid authenticator code is required.'});db.prepare('UPDATE users SET mfa_enabled=0,mfa_secret=NULL WHERE id=?').run(u.id);db.prepare('DELETE FROM mfa_backup_codes WHERE user_id=?').run(u.id);logAudit(db,{assessment_id:null,actor:u.username,action:'mfa_disabled',details:{label:`MFA disabled: ${u.username}`,username:u.username}});res.json({mfa_enabled:false});});
app.post('/api/auth/mfa/verify',(req,res)=>{const v=verifyMfaToken(req.body?.mfa_token);if(!v)return res.status(401).json({error:'MFA verification step expired. Sign in again.'});const u=db.prepare('SELECT * FROM users WHERE id=? AND active=1').get(v.userId);if(!u||!u.mfa_enabled)return res.status(401).json({error:'MFA verification unavailable.'});const key=mfaAttemptKey(u.id), state=mfaFailures.get(key);if(state?.until>Date.now())return res.status(423).json({error:'MFA locked, try again later.'});if(state?.until){mfaFailures.delete(key);}let ok=verifyTOTP(u.mfa_secret,req.body?.code),backup=null;if(!ok){backup=db.prepare('SELECT * FROM mfa_backup_codes WHERE user_id=? AND code_hash=? AND used_at IS NULL').get(u.id,hashBackupCode(req.body?.code));if(backup){db.prepare('UPDATE mfa_backup_codes SET used_at=CURRENT_TIMESTAMP WHERE id=?').run(backup.id);ok=true;}}if(!ok){const count=(state?.count||0)+1;const locked=count>=5;mfaFailures.set(key,{count,until:locked?Date.now()+15*60*1000:0});logAudit(db,{assessment_id:null,actor:u.username,action:'mfa_verify_failure',details:{label:`MFA verification failed: ${u.username}`,username:u.username,fingerprint:mfaFingerprint(req,req.body?.code)}});return res.status(locked?423:401).json({error:locked?'MFA locked, try again later.':'Invalid MFA code.'});}mfaFailures.delete(key);logAudit(db,{assessment_id:null,actor:u.username,action:'mfa_verify_success',details:{label:`MFA verification succeeded: ${u.username}`,username:u.username,backup_code:!!backup}});res.json({token:issueSession(u.id),user:publicUser(u),must_change_password:!!u.must_change_password});});
...
app.post('/api/auth/logout',(req,res)=>{const raw=(req.get('authorization')||'').slice(7);db.prepare('DELETE FROM sessions WHERE token_hash=?').run(crypto.createHash('sha256').update(raw).digest('hex'));res.status(204).end();});
app.put('/api/auth/password',(req,res)=>{const current=req.body?.current_password,newPassword=req.body?.new_password;if(typeof current!=='string'||!current||typeof newPassword!=='string'||newPassword.length<8)return res.status(400).json({error:'current_password is required and new_password must be at least 8 characters.'});if(current===newPassword)return res.status(400).json({error:'New password must differ from current password.'});const row=db.prepare('SELECT * FROM users WHERE id=?').get(req.user.id);if(!row||!verifyPassword(current,row.password_hash))return res.status(400).json({error:'Current password is incorrect.'});const raw=(req.get('authorization')||'').slice(7),keep=crypto.createHash('sha256').update(raw).digest('hex');db.prepare('UPDATE users SET password_hash=?,must_change_password=0,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(hashPassword(newPassword),row.id);db.prepare('DELETE FROM sessions WHERE user_id=? AND token_hash<>?').run(row.id,keep);logAudit(db,{assessment_id:null,actor:req.user.username,action:'password_changed',details:{label:`Password changed: ${req.user.username}`,username:req.user.username}});res.json({ok:true,must_change_password:false});});
app.get('/api/auth/me',(req,res)=>res.json(req.user));
app.get('/api/admin/users',requireRole('admin'),(req,res)=>res.json(db.prepare('SELECT id,username,role,display_name,active,created_at,updated_at FROM users ORDER BY username').all()));
app.post('/api/admin/users',requireRole('admin'),(req,res)=>{const {username,password,role,display_name}=req.body||{};if(!username||!password||!ROLES.includes(role))return res.status(400).json({error:'username, password, and valid role are required.'});try{const r=db.prepare('INSERT INTO users(username,password_hash,role,display_name) VALUES (?,?,?,?)').run(clean(username),hashPassword(password),role,clean(display_name)||null);const created=db.prepare('SELECT id,username,role,display_name,active,created_at,updated_at FROM users WHERE id=?').get(r.lastInsertRowid);logAudit(db,{assessment_id:null,actor:auditActor(req),action:'user_created',details:{label:`User created: ${created.username} (${created.role})`,entity:'user',id:created.id,changed:['username','role','display_name','password']}});res.status(201).json(created);}catch(e){if(String(e.message).includes('UNIQUE'))return res.status(409).json({error:'Username already exists.'});throw e;}});
app.put('/api/admin/users/:id',requireRole('admin'),(req,res)=>{const cur=db.prepare('SELECT * FROM users WHERE id=?').get(req.params.id);if(!cur)return res.status(404).json({error:'User not found.'});const role=req.body.role||cur.role;if(!ROLES.includes(role))return res.status(400).json({error:'Invalid role.'});db.prepare('UPDATE users SET role=?,display_name=?,active=?,password_hash=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(role,req.body.display_name??cur.display_name,req.body.active==null?cur.active:(req.body.active?1:0),req.body.password?hashPassword(req.body.password):cur.password_hash,req.params.id);const after=db.prepare('SELECT id,username,role,display_name,active,created_at,updated_at FROM users WHERE id=?').get(req.params.id);const changed=changedFields({role:cur.role,display_name:cur.display_name,active:cur.active},{role:after.role,display_name:after.display_name,active:after.active},['role','display_name','active']);if(req.body.password)changed.push('password');logAudit(db,{assessment_id:null,actor:auditActor(req),action:'user_updated',details:{label:`User updated: ${after.username}`,entity:'user',id:after.id,changed}});res.json(after);});
...
const select = db.prepare('SELECT id, first_name, last_name, date_of_birth, gender, consent_status, dbhds_flags, notes, created_at, updated_at FROM clients WHERE deleted_at IS NULL ORDER BY last_name COLLATE NOCASE, first_name COLLATE NOCASE');
...
app.get('/api/clients', (_,res)=>res.json(select.all().map(row)));
app.get('/api/clients/:id', (req,res)=>{ const c=row(db.prepare('SELECT id, first_name, last_name, date_of_birth, gender, consent_status, dbhds_flags, notes, created_at, updated_at FROM clients WHERE id=? AND deleted_at IS NULL').get(req.params.id)); if(!c)return res.status(404).json({error:'Client not found.'}); res.json(c); });
app.post('/api/clients', (req,res)=>{const e=validate(req.body);if(e)return res.status(400).json({error:e});const b=req.body;const info=db.prepare('INSERT INTO clients (first_name,last_name,date_of_birth,gender,consent_status,dbhds_flags,notes) VALUES (?,?,?,?,?,?,?)').run(clean(b.first_name),clean(b.last_name),b.date_of_birth||null,clean(b.gender)||null,b.consent_status||'not_started',JSON.stringify(b.dbhds_flags||{}),clean(b.notes)||null);const created=row(db.prepare('SELECT * FROM clients WHERE id=?').get(info.lastInsertRowid));logAudit(db,{assessment_id:null,actor:auditActor(req),action:'client_created',details:{label:`Client created: ${created.first_name} ${created.last_name}`,entity:'client',id:created.id,changed:setFields(created,CLIENT_FIELDS)}});res.status(201).json(created);});
app.put('/api/clients/:id', (req,res)=>{const e=validate(req.body);if(e)return res.status(400).json({error:e});const old=db.prepare('SELECT * FROM clients WHERE id=? AND deleted_at IS NULL').get(req.params.id);if(!old)return res.status(404).json({error:'Client not found.'});const b=req.body;db.prepare('UPDATE clients SET first_name=?,last_name=?,date_of_birth=?,gender=?,consent_status=?,dbhds_flags=?,notes=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(clean(b.first_name),clean(b.last_name),b.date_of_birth||null,clean(b.gender)||null,b.consent_status||'not_started',JSON.stringify(b.dbhds_flags||{}),clean(b.notes)||null,req.params.id);const updated=row(db.prepare('SELECT * FROM clients WHERE id=?').get(req.params.id));logAudit(db,{assessment_id:null,actor:auditActor(req),action:'client_updated',details:{label:`Client updated: ${updated.first_name} ${updated.last_name}`,entity:'client',id:updated.id,changed:changedFields(old,updated,CLIENT_FIELDS)}});res.json(updated);});
app.delete('/api/clients/:id', (req,res)=>{const old=db.prepare('SELECT * FROM clients WHERE id=? AND deleted_at IS NULL').get(req.params.id);if(!old)return res.status(404).json({error:'Client not found.'});const r=db.prepare("UPDATE clients SET deleted_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP WHERE id=? AND deleted_at IS NULL").run(req.params.id);logAudit(db,{assessment_id:null,actor:auditActor(req),action:'client_deleted',details:{label:`Client deleted: ${old.first_name} ${old.last_name}`,entity:'client',id:old.id,changed:['deleted']}});res.status(204).end();});
// --- Assessments ---
...
app.get('/api/clients/:id/assessments', (req,res)=>{ if(!clientExists.get(req.params.id))return res.status(404).json({error:'Client not found.'}); res.json(listAssessments.all(req.params.id)); });
app.post('/api/clients/:id/assessments', (req,res)=>{ if(!clientExists.get(req.params.id))return res.status(404).json({error:'Client not found.'}); const e=validateAssessment(req.body); if(e)return res.status(400).json({error:e}); const b=req.body;
 const info=db.prepare('INSERT INTO assessments (client_id,title,status,assessment_date,assessor,notes) VALUES (?,?,?,?,?,?)').run(req.params.id,clean(b.title),b.status||'draft',b.assessment_date||null,clean(b.assessor)||null,clean(b.notes)||null);
 const a=getAssessment.get(info.lastInsertRowid); logAudit(db,{assessment_id:Number(info.lastInsertRowid),actor:auditActor(req),action:'assessment_created',details:{label:`Assessment created: ${a.title}`,entity:'assessment',id:a.id,changed:setFields(a,ASSESSMENT_FIELDS)}}); res.status(201).json({...a,behavior_count:0,behaviors:[]}); });
app.get('/api/assessments/:id', (req,res)=>{ const a=getAssessment.get(req.params.id); if(!a)return res.status(404).json({error:'Assessment not found.'}); res.json({...a,behaviors:listBehaviors.all(a.id)}); });
app.put('/api/assessments/:id', (req,res)=>{ const old=getAssessment.get(req.params.id); if(!old)return res.status(404).json({error:'Assessment not found.'}); const e=validateAssessment(req.body); if(e)return res.status(400).json({error:e}); const b=req.body;
 db.prepare('UPDATE assessments SET title=?,status=?,assessment_date=?,assessor=?,notes=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(clean(b.title),b.status||'draft',b.assessment_date||null,clean(b.assessor)||null,clean(b.notes)||null,req.params.id);
 const a=getAssessment.get(req.params.id); logAudit(db,{assessment_id:a.id,actor:auditActor(req),action:'assessment_updated',details:{label:`Assessment updated: ${a.title}`,entity:'assessment',id:a.id,changed:changedFields(old,a,ASSESSMENT_FIELDS)}}); res.json({...a,behaviors:listBehaviors.all(a.id)}); });
app.delete('/api/assessments/:id', (req,res)=>{ const old=getAssessment.get(req.params.id); if(!old)return res.status(404).json({error:'Assessment not found.'}); const r=db.prepare("UPDATE assessments SET deleted_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP WHERE id=? AND deleted_at IS NULL").run(req.params.id); logAudit(db,{assessment_id:old.id,actor:auditActor(req),action:'assessment_deleted',details:{label:`Assessment deleted: ${old.title}`,entity:'assessment',id:old.id,changed:['deleted']}}); res.status(204).end(); });
// --- Target behaviors ---
app.get('/api/assessments/:id/target-behaviors', (req,res)=>{ if(!getAssessment.get(req.params.id))return res.status(404).json({error:'Assessment not found.'}); res.json(listBehaviors.all(req.params.id)); });
app.post('/api/assessments/:id/target-behaviors', (req,res)=>{ if(!getAssessment.get(req.params.id))return res.status(404).json({error:'Assessment not found.'}); const e=validateBehavior(req.body); if(e)return res.status(400).json({error:e}); const b=req.body;
 const sc=b.safety_classification||'none';
 const info=db.prepare('INSERT INTO target_behaviors (assessment_id,name,operational_definition,safety_classification,is_safety_concern,baseline_measurement_type) VALUES (?,?,?,?,?,?)').run(req.params.id,clean(b.name),clean(b.operational_definition),sc,sc!=='none'?1:0,b.baseline_measurement_type||null);
 const created=db.prepare('SELECT * FROM target_behaviors WHERE id=?').get(info.lastInsertRowid); logAudit(db,{assessment_id:req.params.id,actor:auditActor(req),action:'behavior_created',details:{label:`Target behavior created: ${created.name}`,entity:'target_behavior',id:created.id,changed:setFields(created,BEHAVIOR_FIELDS)}});
 res.status(201).json(created); });
app.put('/api/target-behaviors/:id', (req,res)=>{ const cur=db.prepare('SELECT * FROM target_behaviors WHERE id=?').get(req.params.id); if(!cur)return res.status(404).json({error:'Target behavior not found.'}); const e=validateBehavior(req.body); if(e)return res.status(400).json({error:e}); const b=req.body;
 const sc=b.safety_classification||'none';
 db.prepare('UPDATE target_behaviors SET name=?,operational_definition=?,safety_classification=?,is_safety_concern=?,baseline_measurement_type=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(clean(b.name),clean(b.operational_definition),sc,sc!=='none'?1:0,b.baseline_measurement_type||null,req.params.id);
 const updated=db.prepare('SELECT * FROM target_behaviors WHERE id=?').get(req.params.id); logAudit(db,{assessment_id:cur.assessment_id,actor:auditActor(req),action:'behavior_updated',details:{label:`Target behavior updated: ${updated.name}`,entity:'target_behavior',id:updated.id,changed:changedFields(cur,updated,BEHAVIOR_FIELDS)}});
 res.json(updated); });
app.delete('/api/target-behaviors/:id', (req,res)=>{ const cur=db.prepare('SELECT * FROM target_behaviors WHERE id=?').get(req.params.id); if(!cur)return res.status(404).json({error:'Target behavior not found.'});
 try { db.prepare('DELETE FROM target_behaviors WHERE id=?').run(req.params.id); }
 catch { return res.status(409).json({error:'This behavior has recorded data points and cannot be deleted.'}); }
 logAudit(db,{assessment_id:cur.assessment_id,actor:auditActor(req),action:'behavior_deleted',details:{label:`Target behavior deleted: ${cur.name}`,entity:'target_behavior',id:cur.id,changed:['deleted']}});
 res.status(204).end(); });
// --- Data points ---
app.get('/api/assessments/:id/data-points', (req,res)=>{ if(!getAssessment.get(req.params.id)) return res.status(404).json({error:'Assessment not found.'}); res.json(db.prepare(`SELECT dp.*, tb.name AS target_behavior_name, tb.is_safety_concern AS target_behavior_safety, tb.safety_classification AS target_behavior_safety_classification FROM data_points dp LEFT JOIN target_behaviors tb ON tb.id=dp.target_behavior_id WHERE dp.assessment_id=? ORDER BY dp.recorded_at DESC, dp.id DESC`).all(req.params.id)); });
app.get('/api/assessments/:id/data-points/summary', (req,res)=>{ if(!getAssessment.get(req.params.id)) return res.status(404).json({error:'Assessment not found.'}); const byBehavior=db.prepare(`SELECT tb.id AS target_behavior_id,tb.name AS target_behavior_name,COUNT(dp.id) AS count FROM target_behaviors tb LEFT JOIN data_points dp ON dp.target_behavior_id=tb.id AND dp.assessment_id=? WHERE tb.assessment_id=? GROUP BY tb.id ORDER BY tb.id`).all(req.params.id,req.params.id); const byMeasurementType=db.prepare('SELECT measurement_type,COUNT(*) AS count FROM data_points WHERE assessment_id=? GROUP BY measurement_type').all(req.params.id); const total=db.prepare('SELECT COUNT(*) AS count FROM data_points WHERE assessment_id=?').get(req.params.id).count; res.json({total,by_behavior:byBehavior,by_measurement_type:byMeasurementType}); });
app.post('/api/assessments/:id/data-points', (req,res)=>{ if(!getAssessment.get(req.params.id)) return res.status(404).json({error:'Assessment not found.'}); const e=validateDataPoint(req.body,req.params.id); if(e)return res.status(400).json({error:e}); const b=req.body; const info=db.prepare('INSERT INTO data_points (assessment_id,target_behavior_id,recorded_at,setting,antecedent,behavior,consequence,measurement_type,value,notes) VALUES (?,?,?,?,?,?,?,?,?,?)').run(req.params.id,Number(b.target_behavior_id),b.recorded_at,clean(b.setting)||null,clean(b.antecedent)||null,clean(b.behavior)||null,clean(b.consequence)||null,b.measurement_type,b.value,clean(b.notes)||null); const created=dataPointRow(info.lastInsertRowid); logAudit(db,{assessment_id:req.params.id,actor:auditActor(req),action:'data_point_created',details:{label:`Data point recorded (${created.target_behavior_name||'behavior'} #${created.target_behavior_id})`,entity:'data_point',id:created.id,changed:setFields(created,DATA_POINT_FIELDS)}}); res.status(201).json(created); });
app.put('/api/data-points/:id', (req,res)=>{ const cur=db.prepare('SELECT * FROM data_points WHERE id=?').get(req.params.id); if(!cur)return res.status(404).json({error:'Data point not found.'}); const e=validateDataPoint(req.body,cur.assessment_id); if(e)return res.status(400).json({error:e}); const b=req.body; db.prepare('UPDATE data_points SET target_behavior_id=?,recorded_at=?,setting=?,antecedent=?,behavior=?,consequence=?,measurement_type=?,value=?,notes=? WHERE id=?').run(Number(b.target_behavior_id),b.recorded_at,clean(b.setting)||null,clean(b.antecedent)||null,clean(b.behavior)||null,clean(b.consequence)||null,b.measurement_type,b.value,clean(b.notes)||null,req.params.id); const updated=dataPointRow(req.params.id); logAudit(db,{assessment_id:cur.assessment_id,actor:auditActor(req),action:'data_point_updated',details:{label:`Data point updated`,entity:'data_point',id:cur.id,changed:changedFields(cur,updated,DATA_POINT_FIELDS)}}); res.json(updated); });
app.delete('/api/data-points/:id', (req,res)=>{ const cur=db.prepare('SELECT * FROM data_points WHERE id=?').get(req.params.id); if(!cur)return res.status(404).json({error:'Data point not found.'}); const r=db.prepare('DELETE FROM data_points WHERE id=?').run(req.params.id); logAudit(db,{assessment_id:cur.assessment_id,actor:auditActor(req),action:'data_point_deleted',details:{label:'Data point deleted',entity:'data_point',id:cur.id,changed:['deleted']}}); res.status(204).end(); });
// --- Function hypotheses ---
...
app.get('/api/assessments/:id/function-hypotheses', (req, res) => {
 if (!getAssessment.get(req.params.id)) return res.status(404).json({ error: 'Assessment not found.' });
 res.json(db.prepare(hypSelect).all(req.params.id).map(hypothesisOut));
});
app.post('/api/assessments/:id/function-hypotheses/generate', (req, res) => {
 if (!getAssessment.get(req.params.id)) return res.status(404).json({ error: 'Assessment not found.' });
 const behaviors = listBehaviors.all(req.params.id);
 const points = db.prepare('SELECT * FROM data_points WHERE assessment_id = ?').all(req.params.id);
 ...
 for (const r of results) upsert.run(req.params.id, r.target_behavior_id, r.function, r.confidence, JSON.stringify(r.evidence));
 logAudit(db, { assessment_id: req.params.id, actor: auditActor(req), action: 'analysis_generated', details: {
 ...
 res.json({ generated: results.length, hypotheses: db.prepare(hypSelect).all(req.params.id).map(hypothesisOut) });
});
app.put('/api/function-hypotheses/:id', (req, res) => {
 const cur = db.prepare('SELECT * FROM function_hypotheses WHERE id = ?').get(req.params.id);
 ...
 const tbName = (db.prepare('SELECT name FROM target_behaviors WHERE id=?').get(cur.target_behavior_id) || {}).name || 'Unknown behavior';
 const fLabel = (code) => (LABELS && LABELS.functions && LABELS.functions[code]) || code;
 db.prepare('UPDATE function_hypotheses SET function = ?, confidence = ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(newFn, confidence, status, req.params.id);
 logAudit(db, { assessment_id: cur.assessment_id, actor: auditActor(req), action: 'hypothesis_overridden', details: {
 ...
 res.json(hypothesisOut(db.prepare(`SELECT fh.*, tb.name AS target_behavior_name FROM function_hypotheses fh JOIN target_behaviors tb ON tb.id = fh.target_behavior_id WHERE fh.id = ?`).get(req.params.id)));
});
// --- Data portability ---
app.get('/api/assessments/:id/export.csv', (req,res) => {
 const out=csvExport(req.params.id); if(!out)return res.status(404).json({error:'Assessment not found.'});
...
app.get('/api/assessments/:id/export.json', (req,res) => { const p=assessmentExport(req.params.id);if(!p)return res.status(404).json({error:'Assessment not found.'});res.set('Content-Disposition',`attachment; filename="${req.query.deidentified==='true'?`Assessment_${req.params.id}`:`Assessment_${p.client?.first_name||'Client'}_${p.client?.last_name||''}`}.json"`);res.json(req.query.deidentified==='true'?deidentify(p):p); });
app.post('/api/assessments/:id/import/csv', (req,res) => { const result=importCsv(req.params.id,req.body);if(result.notFound)return res.status(404).json({error:'Assessment not found.'});if(result.error)return res.status(400).json({error:result.error});const insert=db.prepare('INSERT INTO data_points (assessment_id,target_behavior_id,recorded_at,setting,antecedent,behavior,consequence,measurement_type,value,notes) VALUES (?,?,?,?,?,?,?,?,?,?)');const tx=db.transaction(rows=>{for(const x of rows)insert.run(x.assessment_id,x.target_behavior_id,x.recorded_at,x.setting,x.antecedent,x.behavior,x.consequence,x.measurement_type,x.value,x.notes);});tx(result.valid);logAudit(db,{assessment_id:req.params.id,actor:auditActor(req),action:'data_points_imported',details:{label:`Imported ${result.valid.length} data points`,imported:result.valid.length,rejected_count:result.rejected.length}});res.json({imported:result.valid.length,rejected:result.rejected}); });
// --- FBA report ---
app.get('/api/assessments/:id/report', (req, res) => {
 const report = buildReport(req.params.id);
 ...
 logAudit(db, { assessment_id: req.params.id, actor: auditActor(req), action: 'document_generated', details: {
...
async function sendWord(req, res, buildPayload, buildDocx, label, docType) { const p0 = buildPayload(req.params.id); if (!p0) return res.status(404).json({ error: 'Assessment not found.' }); const deidentified = req.query.deidentified === 'true'; const p = deidentified ? deidentify(p0) : p0; logAudit(db, { assessment_id: req.params.id, actor: auditActor(req), action: 'document_generated', details: { label: `Generated ${label}${deidentified ? ' (de-identified)' : ''}`, document_type: docType, document_type_label: label, deidentified, format: 'docx' } }); try { const buf = await pack(buildDocx(p)); res.set({ 'Content-Type': wordType, 'Content-Disposition': `attachment; filename="${safeFile(p, label)}.docx"` }); return res.send(buf); } catch (e) { console.error('docx export failed', e); return res.status(500).json({ error: 'Unable to generate Word document.' }); } }
app.get('/api/assessments/:id/report.docx', (req,res) => sendWord(req,res,buildReport,docxForReport,'FBA_Report','fba_report'));
...
app.post('/api/assessments/:id/progress-reports', (req,res) => {
 ...
 const payload=buildProgressReport(req.params.id,type,period);
 const info=db.prepare('INSERT INTO progress_reports (assessment_id,period_type,period,period_label,start_date,end_date,payload) VALUES (?,?,?,?,?,?,?)').run(req.params.id,type,period,parsed.label,parsed.start,parsed.end,JSON.stringify(payload));
 logAudit(db,{assessment_id:req.params.id,actor:auditActor(req),action:'progress_report_created',details:{label:`Progress report generated: ${parsed.label}`,entity:'progress_report',id:Number(info.lastInsertRowid),period_type:type,period}});
 res.status(201).json({id:Number(info.lastInsertRowid),...payload});
});
app.get('/api/assessments/:id/progress-reports', (req,res) => {
 if (!getAssessment.get(req.params.id)) return res.status(404).json({error:'Assessment not found.'});
 res.json(db.prepare('SELECT id,period_type,period,period_label,created_at FROM progress_reports WHERE assessment_id=? ORDER BY created_at DESC,id DESC').all(req.params.id));
});
app.get('/api/progress-reports/:id', (req,res) => {
 const row=db.prepare('SELECT * FROM progress_reports WHERE id=?').get(req.params.id); if(!row)return res.status(404).json({error:'Progress report not found.'});
...
app.get('/api/assessments/:id/bip', (req, res) => {
 const doc = buildBip(req.params.id);
...
app.get('/api/assessments/:id/crisis-plan', (req, res) => {
 const doc = buildCrisisPlan(req.params.id);
 if (!doc) return res.status(404).json({ error: 'Assessment not found.' });
 const deidentified = req.query.deidentified === 'true';
 logAudit(db, { assessment_id: req.params.id, actor: auditActor(req), action: 'document_generated', details: {
...
app.get('/api/assessments/:id/data-sheet', (req, res) => {
 const doc = buildDataSheet(req.params.id);
 if (!doc) return res.status(404).json({ error: 'Assessment not found.' });
 const deidentified = req.query.deidentified === 'true';
 logAudit(db, { assessment_id: req.params.id, actor: auditActor(req), action: 'document_generated', details: {
...
app.get('/api/assessments/:id/sign-offs', (req, res) => {
 if (!getAssessment.get(req.params.id)) return res.status(404).json({ error: 'Assessment not found.' });
 ...
 const rows = dt
  ? db.prepare('SELECT * FROM sign_offs WHERE assessment_id=? AND document_type=? ORDER BY id').all(req.params.id, dt)
  : db.prepare('SELECT * FROM sign_offs WHERE assessment_id=? ORDER BY id').all(req.params.id);
 res.json(rows.map(signOffOut));
});
app.post('/api/assessments/:id/sign-offs', (req, res) => {
 if (!getAssessment.get(req.params.id)) return res.status(404).json({ error: 'Assessment not found.' });
 ...
 let info;
 try {
  info = db.prepare('INSERT INTO sign_offs (assessment_id, document_type, signatory_role, signatory_name) VALUES (?, ?, ?, ?)')
 ...
 const row = signOffOut(db.prepare('SELECT * FROM sign_offs WHERE id = ?').get(info.lastInsertRowid));
 logAudit(db, { assessment_id: req.params.id, actor: auditActor(req), action: 'sign_off_created', details: {
...
app.post('/api/sign-offs/:id/sign', (req, res) => {
...
app.delete('/api/sign-offs/:id', (req, res) => {
 ...
 logAudit(db, { assessment_id: row.assessment_id, actor: auditActor(req), action: 'sign_off_revoked', details: {
...
// --- Audit trail ---
app.get('/api/assessments/:id/audit-log', (req, res) => {
 if (!getAssessment.get(req.params.id)) return res.status(404).json({ error: 'Assessment not found.' });
 const limit = Math.min(200, Math.max(1, Number(req.query.limit) || 50));
 const rows = db.prepare('SELECT * FROM audit_log WHERE assessment_id=? ORDER BY id DESC LIMIT ?').all(req.params.id, limit);
 res.json(rows.map((r) => {
  let details = {};
